...
class PlaneTest {
    ...
    void testGetNormal() {
        ...
        assertEquals(new Vector(sqrt3, sqrt3, sqrt3), pl.getNormal(new Point(0, 0, 1)), "TC01: Wrong normal to plane");
    }
    ...
    void testFindIntersections() {
        ...
        assertNull(plane.findIntersections(ray), "TC01: Ray does not intersects the plane EP doesn't work.");


        // =============== Boundary Values Tests ==================
        // TC11: Ray is parallel and included in the plane.
        ray = new Ray(new Point(0, 1, 0), new Vector(1, 0, 0));
        assertNull(plane.findIntersections(ray), "TC11: Ray is parallel and included in the plane BVA doesn't work.");

        // TC12: Ray is parallel and not included in the plane.
        ray = new Ray(new Point(0, 1, 1), new Vector(1, 0, 0));
        assertNull(plane.findIntersections(ray), "TC12: Ray is parallel and not included in the plane BVA doesn't work.");
        ...
        assertNull(plane.findIntersections(ray), "TC14: Ray is orthogonal to the plane and in the plane BVA doesn't work.");

        // TC15: Ray is orthogonal to the plane and after the plane.
        ray = new Ray(new Point(0, 2, -1), new Vector(0, 0, -1));
        assertNull(plane.findIntersections(ray), "TC15: Ray is orthogonal to the plane and after the plane BVA doesn't work.");

        // TC16: Ray begins in the same point which appears as the plane's reference point.
        ray = new Ray(plane.getQ0(), new Vector(1, 1, 0));
        assertNull(plane.findIntersections(ray), "TC16: Ray begins in the same point which appears as the plane's reference point BVA doesn't work.");
        ...
        assertNull(plane.findIntersections(ray), "TC17: Ray begins at the plane but the ray it's not parallel or orthogonal to the plane BVA doesn't work.");
    }
}